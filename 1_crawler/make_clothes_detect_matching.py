...
imsave_path = '/jet/prs/workspace/boundingbox_img_matching/'
detect_path = '/jet/prs/workspace/data'
    ...
    for crop_file in os.listdir(detect_im_dir):
        if crop_file[-3:] == 'jpg':
            ...
            for i in range(min(max_boxes_to_draw, sq_boxes.shape[0])):
                if scores is not None and np.squeeze(scores)[i] > min_score_thresh:
                    ...
                    class_name = class_name[:-1] if class_name.endswith('_') else class_name
                    if class_name is not 'leg':
                        img_name = IMAGE_NAME[:-4] + '_' + class_name + IMAGE_NAME[-4:]
            ...
            if len(resultImList) != 2:
                for file in os.listdir(results_dir):
                    if os.path.isfile(os.path.join(results_dir, file)) and file.startswith(IMAGE_NAME[:-4]):
                        print('delete file = ', file)
                        os.remove(os.path.join(results_dir, file))

for datedir in os.listdir(detect_path):
    detected_img_crop(datedir)