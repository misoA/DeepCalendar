...
def detected_img_crop(datedir, datedir_2):
    detect_im_dir = os.path.join(detect_path, datedir, datedir_2)
    results_dir = os.path.join(imsave_path, datedir, datedir_2)
        ...
        if crop_file[-3:] == 'jpg':
            ...
            for i in range(min(max_boxes_to_draw, sq_boxes.shape[0])):
                if scores is None or np.squeeze(scores)[i] > min_score_thresh:
                    ...
                    img_name = IMAGE_NAME[:-4] + '_' + (class_name[:-1] if class_name.endswith('_') else class_name) + '_' + str(i) + IMAGE_NAME[-4:]
                    plt.imsave(
                            os.path.join(results_dir, img_name)
                            , cv2.cvtColor(img_data, cv2.COLOR_BGR2RGB))


for datedir in os.listdir(detect_path):
    for datedir_2 in os.listdir(os.path.join(detect_path, datedir)):
        print("start category : ", datedir, datedir_2)
        detected_img_crop(datedir, datedir_2)