...
class fashion_dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # [0, 1, 0 ... 0]
        label_num = float(self.label_file[index]['label'])
        #label = np.zeros(5)
        #label[label_num] = 1
        ...
        img_path = os.path.join(self.img_folder,
                                self.label_file[index]['image_1'])
        PIL_image = Image.open(img_path).convert('RGB')
        orig_image = PIL_image
        image_1 = self.transform(orig_image)
        
        # IMAGE_1
        img_path = os.path.join(self.img_folder,
                                self.label_file[index]['image_2'])
        PIL_image = Image.open(img_path).convert('RGB')
        orig_image = PIL_image
        image_2 = self.transform(orig_image)
