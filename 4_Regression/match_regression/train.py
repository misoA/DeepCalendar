...
label_file = []
with open(url + "/deepc/{0}/{0}_train.json".format(keyword), "r") as f:
...
params = list(fashion_cnn.linear1.parameters()) + list(fashion_cnn.linear2.parameters()) + \
        ...
        list(fashion_cnn.bn3.parameters()) + list(fashion_cnn.resnet_1[-3][-1].parameters()) + \
        list(fashion_cnn.resnet_1[-2].parameters()) + list(fashion_cnn.resnet_1[-1].parameters()) + \
        list(fashion_cnn.resnet_2[-3][-1].parameters()) + list(fashion_cnn.resnet_2[-2].parameters()) + \
        list(fashion_cnn.resnet_2[-1].parameters())
...
for epoch in range(1, num_epochs+1):
    for i_step in range(1, total_step+1):
        ...
        images_1, images_2, labels = next(iter(data_loader))
...
#        print(labels)
        ...
        if epoch % save_every == 0:
            torch.save(fashion_cnn.state_dict(), os.path.join('./models', '%s-%d.pkl' % (keyword, epoch)))
