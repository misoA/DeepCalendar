...
with open(url + "/deepc/{0}_regression/{0}_{1}_test.json".format(keyword, cf.tem), "r") as f:
...
f = open('{}_{}_test_result.json'.format(keyword, cf.tem), 'w')
result_list = []
index = 0
...
def denorm_list(t_list):
    denorm_list = []
    for i in range(len(t_list)):
        tem = t_list[i].item()
        tem = denormalize(tem)
        tem = round(tem)
        denorm_list.append(tem)
    return denorm_list
...
for batch_i, data in enumerate(data_loader):
    ...
    output_list = denorm_list(output_list)
    label_list = denorm_list(label_list)
...
if reg_total > 0:
    print('Test Accuracy of Temperature (Overall): %2d%%' % (100 * reg_correct / reg_total))
else:
    print('Test Accuracy of Temperature: N/A (no training examples)')
